...
import org.hunters.server.utils.CharsetFunctions;
...
public class CloseFrame extends ControlFrame {
  ...
  /**
   * 1001 indicates that an endpoint is "going away", such as a org.hunters.server going down, or a browser
   ...
   * the org.hunters.server to negotiate one or more extension, but the org.hunters.server didn't return them in the
   ...
   * org.hunters.server, because it can fail the WebSocket handshake instead.
   */
  public static final int EXTENSION = 1010;
  /**
   * 1011 indicates that a org.hunters.server is terminating the connection because it encountered an unexpected
  ...
  /**
   * 1014 indicates that the org.hunters.server was acting as a gateway or proxy and received an invalid
   * response from the upstream org.hunters.server. This is similar to 502 HTTP Status Code See
   ...
   * endpoint. It is designated for use in applications expecting a status code to indicate that the
   * connection was closed due to a failure to perform a TLS handshake (e.g., the org.hunters.server certificate
   * can't be verified).
   **/
   ...
   */
  private void validateUtf8(ByteBuffer payload, int mark) {
  ...
  private void updatePayload() {
    byte[] by = reason.getBytes(StandardCharsets.UTF_8);
